...
module Bot = functor (S : Soul) -> struct
  ...
  let build_road (inters, rlist) own_color: unit = 
  match best_road inters rlist own_color with 
    | (color, (p1,p2)) -> (goal := ORoad (p1,p2)) 
  ...
  let handle_request ((board, plist, turn, next) : state) : move =
    ...
    match request with
      ...
      | ActionRequest -> 
          (*if we are in the middle phase, then try to build longest road*)
          (if phase_is_not_early structs plist then build_road structs color else ());
