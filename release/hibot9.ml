...
let try_for_res (hexes, ports) (inters, rlist) robberloc needo color plist goal trades_made turn:  strategy =
  ...
  let choose_init_move (inters, roads) board color  plist: line = 
    let weighted_inters = sort_locs inters board !res_tiles plist in
   ...
   let town_builder (inters, roads) board color plist : unit = 
    ...
    (goal:= town_move (inters, roads) !res_tiles board (b,w,o,g,l) color plist)
    ...
    match request with
      ...
      | ActionRequest -> 
        ...
        (if (!goal = NA && !need_card) then goal:=OCard);
          ...
          (if (!goal = NA && (get_num_vp_us color plist (inters, roads)) >= cWIN_CONDITION-4 && not (have_largest_army plist (inters, roads) color)
          && (delta_largest_army plist (inters, roads) color) <= 2) then
           ...
           (* If trying to get another road for trophy, then *)
            ...
            let () = potent_road:= (if (!goal = NA) && get_num_roads color roads < cMAX_ROADS_PER_PLAYER &&
               get_num_vp_us color plist (inters, roads) >= cWIN_CONDITION - 5 && not (have_longest_road plist (inters, roads) color)
          ...
          if not (can_pay_cost !res_needed color plist) then
            match try_for_res map structs robber !res_needed color plist !goal trades_tried turn with
              ...
              | SOfferTrade tr -> Action (DomesticTrade tr)
