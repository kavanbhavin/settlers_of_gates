...
(* Return whether or not building a road would increase our longest road.
	If so, Some of that road. Otherwise None. Also include weight.*)
let extend_road_w color (inters, roads) : (road * int) option = 
	let long_road = longest_road' color roads inters in 
	let poss_roads = all_possible_roads (inters, roads) color in
	let weighted_roads = List.map (fun l -> 
		let new_roads = (color,l)::roads in 
	...
	match sorted_roads with
		| [] -> None
		| (r, w)::_ -> if (w <= 0) then None else Some (r, w)

(* Return whether or not building a road would increase our longest road.
	Checks 2 in advance. If so, Some of that road. Otherwise None. *)
let extend_road_d2 color (inters, roads) : (road option * road option) = 
	let long_road = longest_road' color roads inters in 
	let poss_roads = all_possible_roads (inters, roads) color in
	let weighted_roads = List.map (fun l -> 
		let new_roads = (color,l)::roads in 
		match (extend_road_w color (inters, new_roads)) with
			| None -> let weight = (longest_road' color new_roads inters) - long_road in
				(Some (color, l), None, weight)
			| Some (road, weight) -> 
				(Some (color, l), Some road, weight)) poss_roads in
	let sorted_roads = List.sort (fun (_, _, a) (_,_, b) -> b - a) weighted_roads in
	match sorted_roads with
		| [] -> (None, None)
		| (r1, r2, w)::_ -> if (w <= 0) then (None, None) else (r1, r2)
