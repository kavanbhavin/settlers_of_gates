...
let handle_move' ((map, structs, deck, discard, robber), 
  ...
  | InitialMove l -> begin
    match req with
    | InitialRequest ->
      let ((intersections', roads'), road', plist') = (init_request board l color plist) in 
      let (color', req') = update_init_turn (List.length roads') color 
  		in (((get_winner plist' (intersections', roads')), ((map, (intersections', roads'), deck, discard, robber), plist', (new_turn color'), (color', req'))),
  			(InitialMove road'))
    | _ -> default_move game
  end
  | RobberMove rm -> begin 
    ...
    | RobberRequest -> 
    	begin match do_robber_move turn.active rm structs robber plist with 
          | Some (robber', plist', move') -> (((get_winner plist' structs), ((map, structs, deck, discard, robber'), plist', turn, (color, ActionRequest))),
          	move')
          | None -> 
  ...
  | DiscardMove (dm) -> begin
  	...
  	| DiscardRequest ->
  		...
  		(((get_winner plist' structs), (board, plist', turn, (next'))), DiscardMove cost_found)
  	| _ -> default_move game
  end
    | TradeResponse res -> begin
    	...
    	| TradeRequest ->
	    	...
	    	(((get_winner plist' structs), (board, plist',turn',(origin, ActionRequest))), m)
	    | _ -> default_move game
    end
    | Action act -> begin
    	match req with
        ...
        | RollDice -> let (next', plist', turn') = roll_dice plist board color turn
      		in (((get_winner plist' structs), (board, plist', turn', next')), Action RollDice)
	    | DomesticTrade tr ->
	    	...
	    	let (turn', next') = update_turn_before_trade turn (color,req) tr color plist in
	    		(((get_winner plist structs), (board, plist, turn', next')), m)
	    | BuyBuild build -> 
        ...
        | EndTurn ->
          begin match end_turn turn plist with 
              | Some (turn', plist') -> 
              		(((get_winner plist' structs), (board, plist', turn', (turn'.active , ActionRequest))), m)
              | None -> default_move game 
          end 
    	| MaritimeTrade mt ->
          begin match make_sea_trade map structs color mt plist with
           	  | Some plist' ->
           	  	(((get_winner plist' structs), (board, plist', turn, (color, req))), m)
           	  | None -> default_move game
    	  end end
    	| _ -> default_move game
    	end
